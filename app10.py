            ...
            contents=
{
  ...
  "body":
    ...
    +
        ...
        reply_message2 = TemplateSendMessage(
        alt_text='要繼續點餐嗎？',
        template=ButtonsTemplate(
            thumbnail_image_url='https://pgw.udn.com.tw/gw/photo.php?u=https://uc.udn.com.tw/photo/2022/04/21/0/16706436.jpg&x=0&y=0&sw=0&sh=0&sl=W&fw=800&exp=3600&w=930&nt=1',
            title='在！你想幹嘛呢',
        ...
        try:
            ...
            line_bot_api.reply_message(event.reply_token, reply_message2)
        except:
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text='Sorry, 菜單上沒有呦'))
            
            line_bot_api.reply_message(event.reply_token, reply_message2)  
    
    elif re.match(message, "點完了"):
        n = 0
        for i in range(len(stored_list)):
            n += dict[stored_list[i]]
        line_bot_api.reply_message(event.reply_token, TextSendMessage(text="你總共點了 {} 總共熱量為 {} 大卡".format(stored_list, n)))
        stored_list.clear() 
